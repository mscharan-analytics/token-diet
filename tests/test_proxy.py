...
def test_proxy_anthropic_messages_flow(mock_post):
    ...
    mock_response_2.json.return_value = {
        ...
        "content": [{"type": "text", "text": "Completed! Segment Fault was located and analyzed."}],
    }
    ...
    payload = {
        ...
        "max_tokens": 1000,
        "messages": [{"role": "user", "content": f"Review this crash log:\n{huge_log}"}],
    }
...
def test_proxy_openai_completions_flow(mock_post):
    ...
    mock_response_2.json.return_value = {
        ...
        "choices": [
            {
                "index": 0,
                "message": {"role": "assistant", "content": "Completed! Successfully resolved details."},
                "finish_reason": "stop",
            }
        ],
    }
    ...
    payload = {"model": "gpt-4o", "messages": [{"role": "user", "content": f"Review this log:\n{huge_log}"}]}
    ...
    def side_effect_handle(tool_input):
        retrieval_id = tool_input.get("retrieval_id")
        mock_response_1.json.return_value["choices"][0]["message"]["tool_calls"][0]["function"]["arguments"] = (
            json.dumps({"retrieval_id": retrieval_id})
        )
        mock_response_1.content = json.dumps(mock_response_1.json.return_value).encode("utf-8")
        return original_handle(tool_input)
